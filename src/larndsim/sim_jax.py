...
def simulate_signals(params, electrons, mask_indices, pix_renumbering, unique_pixels, response, rngkey, fields):
    ...
    nticks_wf = int(params.time_interval[1]/params.t_sampling) #Adding one first element to serve as a garbage collector
    wfs = jnp.zeros((npixels, nticks_wf))

    readout_tshift = (t0/params.t_sampling).astype(int) #Start tick from distance to the end of the cathode, corresponds to the shift with respect to the response readout start
    response_cum = jnp.cumsum(response, axis=-1)
    wfs = accumulate_signals(wfs, currents_idx, electrons_renumbered[:, fields.index("n_electrons")], response, response_cum, pix_renumbering, readout_tshift, params.signal_length)
    integral, ticks = get_adc_values(params, wfs, rngkey)

    adcs = digitize(params, integral)
    return adcs, ticks, readout_tshift, wfs
