...
SAMPLING_STEP=0.005 #0.0005 #0.005 cm
...
python3 -m optimize.example_run \
    ...
    --no-noise \
    --data_seed ${DATA_SEED} \
    --out_label p_5000_6par_match_z_diff_curr_no_noise_e_sampling_${SAMPLING_STEP}cm_seed_strategy_${SEED_STRATEGY}_grad_clip${MAX_CLIP_NORM_VAL}_bt${BATCH_SIZE}_tgtsd${TARGET_SEED}_dtsd${DATA_SEED}_adam_${LOSS}_${UUID} \
    --test_name fit_noise \
    --seed ${TARGET_SEED} \
    --optimizer_fn Adam \
    --random_ntrack \
