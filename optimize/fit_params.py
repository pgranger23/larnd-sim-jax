    ...
    def get_simulated_target(self, tracks, i, regen=False):
        ...
        if regen or not os.path.exists(fname):
            if self.current_mode == 'lut':
                ...
                ref_adcs, ref_unique_pixels, ref_ticks, ref_pix_matching, ref_electrons, ref_ticks_electrons = simulate_parametrized(self.target_params, tracks, self.track_fields, i+1, self.diffusion_in_current_sim) #Setting a different random seed for each target
